/**
 * PromptPayQR - Utility class for generating PromptPay QR payloads and checksums.
 * Original logic from diewland.github.io, refactored for modular use.
 */
class PromptPayQR {
    /**
     * Calculate CRC16-CCITT (XModem) checksum for a given string.
     * @param {string} s - The input string to calculate checksum for.
     * @returns {string} - The 4-character uppercase hexadecimal checksum.
     */
    static checksum(s) {
        const crcTable = [
        ...
        const chksum = ((crc ^ 0) & 0xFFFF).toString(16);
        return ('0000' + chksum).slice(-4).toUpperCase();
    }

    /**
     * Generate PromptPay QR payload string.
     * @param {string} acc_id - PromptPay ID (phone, national ID, or e-wallet).
     * @param {number|string} [amount] - Amount in Baht (optional).
